#!/usr/bin/env python3
"""
RAG Command Line Interface with Query Reformulation
====================================================
Interactive chat with intelligent query handling!
...
from rag_pipeline_enhanced import RAGPipeline
...
def print_banner():
    ...
    banner = """
╔═══════════════════════════════════════════════════╗
║         LOCAL RAG SYSTEM v2.0                     ║
║   With Intelligent Query Reformulation!          ║
╚═══════════════════════════════════════════════════╝
"""
...
Features:
  • Automatic query clarification for ambiguous questions
  • Query expansion with related terms
  • Confidence-based suggestions for improvement
...
def print_topics(pipeline: RAGPipeline):
    """Show available topics."""
    print_color("\n📚 Available Topics:", Colors.CYAN)
    for i, topic in enumerate(pipeline.AVAILABLE_TOPICS, 1):
        print(f"  {i}. {topic}")
    print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="RAG System v2 - Chat with query reformulation"
    )
    parser.add_argument(
        "--model", "-m",
        required=True,
    ...
    parser.add_argument(
        "--no-reformulation",
        action="store_true",
        help="Disable query reformulation features"
    )
    ...
    if not args.no_reformulation:
        print_color("✨ Query reformulation ENABLED", Colors.GREEN)
        print_color("   The system will ask for clarification on ambiguous queries\n", Colors.YELLOW)
    ...
    # Create the pipeline with reformulation
        ...
        try:
            ...
            if user_input.startswith("/"):
                ...
                if cmd in ["/quit", "/exit", "/q"]:
                    ...
                    print_topics(pipeline)
                
                elif cmd == "/history":
                    print_color("\n" + pipeline.get_conversation_summary(), Colors.MAGENTA)
                    print()
                
                elif cmd == "/clear":
                    pipeline.conversation_history = []
                    print_color("Conversation history cleared!", Colors.YELLOW)
            ...
            is_clarification = (
                "need more information" in response.answer.lower() or
                "could you please clarify" in response.answer.lower() or
                "would you like to" in response.answer.lower()
            )
            
            # Display the answer
            print()
            if is_clarification:
                print_color("🔍 Clarification Needed:", Colors.MAGENTA + Colors.BOLD)
            else:
                print_color("Answer:", Colors.GREEN + Colors.BOLD)
            
            print(response.answer)
            
            # Display confidence bar (only for actual answers)
            if response.confidence > 0:
                ...
                print_color(f"\nConfidence: [{bar}] {response.confidence:.0%}", conf_color)
